...
// Cost-effective polling hook with navigation-aware pausing
function useSmartPolling({
  ...
  const pollData = useCallback(async () => {
    if (!enabled || isPolling || isNavigating) return;
  ...
  }, [userId, userRole, filters, pagination, enabled, onDataUpdate, isPolling, isNavigating]);

  // Pause polling when page is hidden or during navigation
  useEffect(() => {
    const handleVisibilityChange = () => {
      setIsNavigating(document.hidden);
    };
  ...
  useEffect(() => {
    ...
    // Initial poll with a small delay to avoid interference with navigation
    const initialTimeout = setTimeout(pollData, 1000);

    // Adaptive polling: more conservative intervals to prevent interference with navigation
    const isActivePage = filters.status === 'processing' || filters.status === 'pending';
    const interval = setInterval(pollData, isActivePage ? 8000 : 20000);

    return () => {
      clearTimeout(initialTimeout);
      clearInterval(interval);
    };
  }, [pollData, enabled, filters.status, isNavigating]);

  return {
    isPolling,
    refreshNow: pollData,
    lastUpdate: lastSuccessfulUpdate,
    status: !enabled || isNavigating ? 'paused' : 
            hasError ? 'error' : 
            isPolling ? 'updating' : 'active',
    hasError
  };
}
...
export default function TransactionsPageWithAdvancedFilters() {
  ...
  // Smart polling setup (restart on locale change)
  const { status: pollingStatus, lastUpdate, hasError } = useSmartPolling({
        ...
        <header className="home-header">
          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center w-full gap-4">
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2 sm:gap-2">
              {/* Auto-refresh indicator (shows actual status) */}
