...
export default function WithdrawListPageOptimized() {
  ...
  // Filter and memoize display transactions
  ...
  const handleNewTransactionReceived = useCallback(
    (newTransaction: Transaction) => {
      ...
      if (isAssignedToCurrentUser) {
        ...
        toast({
          variant: "default",
          description: `✅ New order assigned: ${newTransaction.odrId} (Added to your queue)`,
        });
      } else {
        ...
        toast({
          variant: "default",
          description: `🆕 New withdrawal order received: ${newTransaction.odrId} (Not assigned to you)`,
        });
      }
    ...
    [refreshTotalCount, toast, setNewOrdersAvailable, setTransactions]
  ); // Removed volatile dependencies
  ...
  }, [fetchInitialData, refreshTotalCount, setNewOrdersAvailable]); // Removed volatile dependencies
  ...
  const handleStatusUpdate = useCallback(
    ...
    [toast, setTransactions, manualRefresh]
  );
    ...
    try {
      ...
      if (unassignedTransactions.length === 0) {
        toast({
          variant: "default",
          description:
            "All pending transactions are already assigned to users.",
        });
        return;
      }
      ...
      toast({
        variant: "default",
        description: `Starting background assignment of ${unassignedTransactions.length} withdrawals...`,
      });
        ...
        try {
          ...
          for (let batchIndex = 0; batchIndex < batches.length; batchIndex++) {
            ...
            if (
              ...
              toast({
                variant: "default",
                description: `Progress: ${processed}/${totalCount} assignments processed (${successCount} successful)`,
              });
            }
          ...
          if (successCount > 0 && failureCount === 0) {
            toast({
              variant: "default",
              description: `✅ Background assignment completed! Successfully assigned all ${successCount} withdrawal(s) to users.`,
            });
          } else if (successCount > 0 && failureCount > 0) {
            toast({
              variant: "default",
              description: `✅ Background assignment completed! Assigned ${successCount} withdrawal(s) successfully. ${failureCount} failed.`,
            });
          } else {
            toast({
              variant: "destructive",
              description: `❌ Background assignment failed. ${failureCount} withdrawal(s) could not be assigned. No available users or system error.`,
            });
          }
        } catch {
          toast({
      ...
      toast({
        variant: "destructive",
        description: "Failed to start bulk assignment. Please try again.",
      });
      setAssigningTransactions(new Set());
    }
  }, [toast, manualRefresh]); // Removed volatile dependencies - using refs instead
